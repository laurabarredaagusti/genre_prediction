...
class Extract:
    ...
    def define_track_id(self):
        ...
        # We convert the song name into a query and use it to call the API and get the track id
        query = '?q=' + self.song_name + '&type=track'
        search_get = requests.get(BASE_URL + 'search' + query, headers= self.headers)
        search_get = search_get.json()
        
        self.request_completed = False

        try:
            self.track_id = search_get['tracks']['items'][0]['id']
            self.request_completed = True
        except:
            st.markdown('Invalid name, please enter a different name')
    ...
    def extract_all_features(self):
        ...
        self.api_callextract_track_audio_features
